        ...
        if self.cert_data:
            ...
            db.execute(
                """UPDATE certificates SET certificate_number=?, product_type=?,
                   total_amount=?, remaining_amount=?, issue_date=?, status=?
                   WHERE id=?""",
                (cert_number, product_type, total_amount, new_remaining,
                 issue_date, status, self.cert_data["id"])
            )
        else:
            ...
            cert_id = db.execute(
                """INSERT INTO certificates
                   (company_id, certificate_number, product_type, total_amount,
                    remaining_amount, unit_id, issue_date, status)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (company_id, cert_number, product_type, total_amount,
                 remaining, unit_id, issue_date, status)
            )
            ...
            if debt > 0:
                db.execute(
                    """UPDATE exit_items SET debt_settled = 1, certificate_id = ?
                       WHERE exit_permit_id IN (
                           SELECT ep.id FROM exit_permits ep WHERE ep.company_id = ?
                       ) AND is_debt = 1 AND debt_settled = 0 AND unit_id = ?""",
